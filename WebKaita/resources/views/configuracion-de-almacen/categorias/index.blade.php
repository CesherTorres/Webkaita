...
@section('content')
        ...
        <div class="col-12 col-md-6">
            <div class="text-start text-md-end pb-3">
                <div class="btn-group me-2" role="group" aria-label="Basic example">
                    <a href="{{url('categorias/export/reporte-excel')}}" class="btn btn-dark border-white btn-sm px-4"><i class="bi bi-file-earmark-excel me-2"></i>CSV</a>
                    <a href="{{route('print-categories.pdf')}}" target="bank" class="btn btn-dark border-white btn-sm px-4"><i class="bi bi-file-earmark-pdf me-2"></i>PDF</a>
                </div>
                <small>
                                ...
                                <td class="align-middle">                                        
                                    <div class="text-center">
                                        <form method="POST" action="{{ route('categorias.destroy',$categoria->slug) }}" class="form-delete">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="btn btn-secondary text-white btn-sm " data-bs-toggle="modal" data-bs-target="#editcategorias{{$categoria->slug}}"><i class="bi bi-pencil-square"></i></button>
                                            <button type="submit" class="btn btn-secondary text-white btn-sm"><i class="bi bi-trash-fill"></i></button>        
                                        </form>
                                    </div>    
                                </td>
                            </tr>
                            @include('configuracion-de-almacen.categorias.edit')
                        @endforeach
                    </tbody>
            </table>
        </div>
    ...
    @include('configuracion-de-almacen.categorias.create')
@endsection
