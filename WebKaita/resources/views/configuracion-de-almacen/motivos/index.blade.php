...
@section('content')
                    ...
                    <tbody>
                        @foreach($motivos as $motivo)
                            <tr>
                                ...
                                <td class="fw-light align-middle">{{$motivo->tipo_motivo}}</td>
                                ...
                                {{-- <td class="align-middle">                                        
                                    <div class="text-center">
                                        <form method="POST" action="{{ route('motivos.destroy',$motivo->slug) }}" class="form-delete">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="btn btn-secondary text-white btn-sm " data-bs-toggle="modal" data-bs-target="#editmotivo{{$motivo->slug}}"><i class="bi bi-pencil-square"></i></button>
                                            <button type="submit" class="btn btn-secondary text-white btn-sm"><i class="bi bi-trash-fill"></i></button>        
                                        </form>
                                    </div>    
                                </td> --}}
                            </tr>
                            {{-- @include('configuracion-de-almacen.motivos.edit') --}}
                        @endforeach
                    </tbody>
            </table>
        </div>
    ...
    @include('configuracion-de-almacen.motivos.create')
@endsection

@section('js')
    <!--sweet alert agregar-->
    @if(session('addmotivo') == 'ok')
    <script>
        Swal.fire({
        icon: 'success',
