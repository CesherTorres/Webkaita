...
@section('content')
                    ...
                    <tbody>
                        @foreach($medidas as $medida)
                            <tr>
                                ...
                                <td class="fw-light align-middle">{{$medida->descripcion}}</td>
                                <td class="align-middle">                                        
                                    <div class="text-center">
                                        <form method="POST" action="{{ route('medidas.destroy',$medida->slug) }}" class="form-delete">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="btn btn-secondary text-white btn-sm " data-bs-toggle="modal" data-bs-target="#editunmedidas{{$medida->slug}}"><i class="bi bi-pencil-square"></i></button>
                                            <button type="submit" class="btn btn-secondary text-white btn-sm"><i class="bi bi-trash-fill"></i></button>        
                                        </form>
                                    </div>    
                                </td>
                            </tr>
                            @include('configuracion-de-almacen.medidas.edit')
                        @endforeach
                    </tbody>
            </table>
        </div>
    ...
    @include('configuracion-de-almacen.medidas.create')
@endsection

@section('js')
    <!--sweet alert agregar-->
    @if(session('addmedida') == 'ok')
    <script>
        Swal.fire({
        icon: 'success',
