    ...
    <!-- Encabezado -->
        <div class="row pt-5">
            ...
            <div class="col-lg-3 d-flex">
                <a href="{{url('productos/create')}}" class="btn btn-primary w-100 align-self-center text-white mb-2 mb-md-0">
        ...
        <div class="col-12 col-md-6">
            <div class="text-start text-md-end pb-3">
                <div class="btn-group me-2" role="group" aria-label="Basic example">
                    <a href="{{url('productos/export/reporte-excel')}}" class="btn btn-dark border-white btn-sm px-4"><i class="bi bi-file-earmark-excel me-2"></i>CSV</a>
                    <a href="{{route('print-productos.pdf')}}" target="bank" class="btn btn-dark border-white btn-sm px-4"><i class="bi bi-file-earmark-pdf me-2"></i>PDF</a>
                </div>
                <small>
                    ...
                    <tr>
                        ...
                        <tbody>
                            @foreach($productos as $producto)
                                <tr>
                                    <td class="fw-light align-middle">{{$producto->id}}</td>
                                    <td class="fw-light align-middle">{{$producto->codigo}}</td>
                                    <td class="fw-light align-middle">{{$producto->name}}</td>
                                    <td class="fw-light align-middle">{{$producto->marca->name}}</td>
                                    <td class="fw-light align-middle">{{$producto->category->name}}</td>
                                    <td class="fw-light align-middle">{{$producto->medida->name}}</td>
                                    <td class="fw-light align-middle">{{$producto->precio}}</td>
                                    <td class="align-middle">                                        
                                        <div class="text-center">
                                            <form method="POST" action="{{ route('productos.destroy',$producto->slug) }}" class="form-delete">
                                                @csrf
                                                @method('DELETE')
                                                <a href="{{url("productos/$producto->slug")}}" class="btn btn-secondary text-white btn-sm text-decoration-none"><i class="bi bi-eye-fill"></i></a>
                                                <a href="{{url("productos/$producto->slug/edit")}}" type="button" class="btn btn-secondary text-white btn-sm " ><i class="bi bi-pencil-square"></i></a>
