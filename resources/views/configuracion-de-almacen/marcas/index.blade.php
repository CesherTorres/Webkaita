...
@section('content')
                                ...
                                <td class="align-middle">                                        
                                    <div class="text-center">
                                        <form method="POST" action="{{ route('marcas.destroy',$marca->slug) }}" class="form-delete">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="btn btn-secondary text-white btn-sm " data-bs-toggle="modal" data-bs-target="#editmarcas{{$marca->slug}}"><i class="bi bi-pencil-square"></i></button>
                                            <button type="submit" class="btn btn-secondary text-white btn-sm"><i class="bi bi-trash-fill"></i></button>        
                                        </form>
                                    </div>    
                                </td>
                            </tr>
                            @include('configuracion-de-almacen.marcas.edit')
                        @endforeach
                    </tbody>
            </table>
        </div>
    ...
    <br>
    {{-- Fin contenido --}}     
    @include('configuracion-de-almacen.marcas.create')
@endsection
@section('js')
    <!--sweet alert agregar-->
    @if(session('addmarca') == 'ok')
    <script>
        Swal.fire({
        icon: 'success',
