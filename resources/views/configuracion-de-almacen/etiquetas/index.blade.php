...
@section('content')
                ...
                <thead class="bg-light">
                    ...
                    <tbody>
                        @foreach($etiquetas as $etiqueta)
                            <tr>
                                <td class="fw-light align-middle">{{$etiqueta->id}}</td>
                                <td class="fw-light align-middle">{{$etiqueta->name}}</td>
                                <td class="fw-light align-middle">{{$etiqueta->descripcion}}</td>
                                <td class="align-middle">                                        
                                    <div class="text-center">
                                        <form method="POST" action="{{ route('etiquetas.destroy',$etiqueta->slug) }}" class="form-delete">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="btn btn-secondary text-white btn-sm " data-bs-toggle="modal" data-bs-target="#editetiquetas{{$etiqueta->slug}}"><i class="bi bi-pencil-square"></i></button>
                                            <button type="submit" class="btn btn-secondary text-white btn-sm"><i class="bi bi-trash-fill"></i></button>        
                                        </form>
                                    </div>    
                                </td>
                            </tr>
                            @include('configuracion-de-almacen.etiquetas.edit')
                        @endforeach
                    </tbody>
            </table>
        </div>
    ...
    @include('configuracion-de-almacen.etiquetas.create')
@endsection

@section('js')
    <!--sweet alert agregar-->
    @if(session('addetiqueta') == 'ok')
    <script>
        Swal.fire({
        icon: 'success',
